...
export const GET = withSecurity({
  ...
  try {
    ...
    if (id) {
        const { data } = await supabase.from('admin_audit_logs').select('*').eq('id', id).single();
...
export const POST = withSecurity({
  ...
  try {
    ...
    const { data, error } = await supabase.from('admin_audit_logs').insert(validatedData).select().single();
...
export const PUT = withSecurity({
  ...
  try {
    ...
    const { data, error } = await supabase.from('admin_audit_logs').update(validatedUpdates).eq('id', id).select().single();
    if (error) throw error;
    return NextResponse.json({ success: true, data });
  } catch (err: any) { 
    return NextResponse.json({ error: err.message }, { status: 500 }); 
...
export const DELETE = withSecurity({
  ...
  try {
    ...
    const { error } = await supabase.from('admin_audit_logs').delete().eq('id', id);
    if (error) throw error;
