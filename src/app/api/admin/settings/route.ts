...
export async function GET(request: Request) {
  try {
    ...
    const { data } = await supabase.from('system_feature_flags').select('*').order('flag_key', { ascending: true });
...
export async function POST(request: Request) {
  try {
    ...
    const { data } = await supabase.from('system_feature_flags').upsert(body).select().single();
