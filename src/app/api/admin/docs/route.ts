...
export async function GET(request: Request) {
  try {
    ...
    const { data } = await supabase.from('platform_documentation').select('*').order('updated_at', { ascending: false });
...
export async function POST(request: Request) {
  try {
    ...
    const { data } = await supabase.from('platform_documentation').insert(body).select().single();
