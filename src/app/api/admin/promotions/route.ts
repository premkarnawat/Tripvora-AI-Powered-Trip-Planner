...
export async function GET(request: Request) {
  try {
    ...
    const { data, error } = await supabase.from('promotional_coupons').select('*').order('created_at', { ascending: false });
...
export async function POST(request: Request) {
  try {
    ...
    const { data, error } = await supabase.from('promotional_coupons').insert(body).select().single();
