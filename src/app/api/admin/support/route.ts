...
export const POST = withSecurity({
  ...
  try {
    ...
    const { data, error } = await supabase.from('support_tickets').insert(validatedData).select().single();
...
export const PUT = withSecurity({
  ...
  try {
    ...
    const { data, error } = await supabase.from('support_tickets').update(validatedUpdates).eq('id', id).select().single();
    if (error) throw error;
    return NextResponse.json({ success: true, data });
  } catch (err: any) { 
    return NextResponse.json({ error: err.message }, { status: 500 }); 
