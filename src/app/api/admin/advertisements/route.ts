...
export async function GET(request: Request) {
  try {
    ...
    const { data, error } = await supabase.from('advertisement_campaigns').select('*').order('created_at', { ascending: false });
    if (error && error.code !== '42P01') console.warn("DB notice:", error.message);
...
export async function POST(request: Request) {
  try {
    ...
    const { data, error } = await supabase.from('advertisement_campaigns').insert(body).select().single();
