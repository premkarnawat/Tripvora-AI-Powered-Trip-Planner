...
export async function GET(request: Request) {
  try {
    ...
    const { data } = await supabase.from('legal_documents').select('*').order('updated_at', { ascending: false });
...
export async function POST(request: Request) {
  try {
    ...
    const { data } = await supabase.from('legal_documents').insert(body).select().single();
