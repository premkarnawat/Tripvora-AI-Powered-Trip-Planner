...
export const GET = withSecurity({
  ...
  try {
    ...
    const { data } = await supabase.from('legal_documents').select('*').order('created_at', { ascending: false }).limit(100);
...
export const POST = withSecurity({
  ...
  try {
    ...
    const { data, error } = await supabase.from('legal_documents').insert(validatedData).select().single();
...
export const PUT = withSecurity({
  ...
  try {
    ...
    const { data, error } = await supabase.from('legal_documents').update(validatedUpdates).eq('id', id).select().single();
    if (error) throw error;
    return NextResponse.json({ success: true, data });
  } catch (err: any) { 
    return NextResponse.json({ error: err.message }, { status: 500 }); 
