...
export async function GET(request: Request) {
  try {
    ...
    const { data } = await supabase.from('video_content').select('*').order('created_at', { ascending: false });
...
export async function POST(request: Request) {
  try {
    ...
    const { data } = await supabase.from('video_content').insert(body).select().single();
