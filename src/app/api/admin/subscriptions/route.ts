...
export async function GET(request: Request) {
  try {
    ...
    const { data } = await supabase.from('platform_subscriptions').select('*').order('created_at', { ascending: false });
...
export async function POST(request: Request) {
  try {
    ...
    const { data } = await supabase.from('platform_subscriptions').insert(body).select().single();
